...
const timeRangeMap = { 'ngày': 'day', 'tuần': 'week', 'tháng': 'month', 'năm': 'year', 'all': 'all' };
function getApiTimeRange(v) { return timeRangeMap[v] || v; }
function t(text) { return window.translationMapForCharts?.[text] || text; }
...
function downloadChart(chart, filename, type) {
  ...
  const hasData = chart.data.labels?.length > 0 &&
  ...
  if (type === "png") {
    ...
    let csv = "\uFEFF" + esc(t("Label")) + "," + datasets.map(ds => esc(ds.label)).join(",") + "\n";
...
function drawDemographicsChart(data) {
    ...
    options: {
        ...
        tooltip: {
          ...
          callbacks: {
            title: ctxArr => {
              ...
              const hometown = data.find(d => t(d.productName) === prod)?.hometown || t("N/A");
      ...
      scales: {
        ...
        y: {
          ...
          ticks: { stepSize: 1, color: getChartColor(THEME_COLORS.light.datalabel, THEME_COLORS.dark.datalabel), callback: v => Number.isInteger(v) ? v : null },
          grid: { color: getChartColor(THEME_COLORS.light.grid, THEME_COLORS.dark.grid) }
        }
      }
    }
  ...
  if (Array.isArray(data) && data.length) {
    const products = [...new Set(data.map(i => t(i.productName || "Unknown")))];
    ...
    const datasets = ageGroups.map((age, idx) => ({
      ...
      data: products.map(prod =>
        data.filter(d => t(d.productName) === prod && d.ageGroup === age)
  ...
  myTopFavoriteChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        borderColor: getChartColor(THEME_COLORS.light.yellow, THEME_COLORS.dark.yellow).replace("0.7", "1"),
        borderWidth: 1,
        barThickness: 20
      }]
    },
...
async function refreshDemographicsChart(showAlert = true) {
  const sel = document.getElementById("demographicsTimeRange")?.value || "month";
  const tr = getApiTimeRange(sel);
  ...
  if (!validateFilterInputs(sd, ed)) return;
  let url = `${ocopApi}analytics-userdemographics?timeRange=${encodeURIComponent(tr)}`;
  if (sd) url += `&startDate=${encodeURIComponent(sd)}`;
  if (ed) url += `&endDate=${encodeURIComponent(ed)}`;
  try {
    const res = await fetch(url);
...
async function refreshTopInteractionChart(showAlert = true) {
  const sel = document.getElementById("topInteractionTimeRange")?.value || "month";
  const tr = getApiTimeRange(sel);
  ...
  if (!validateFilterInputs(sd, ed)) return;
  let url = `${ocopApi}analytics-getTopProductsByInteractions?top=5&timeRange=${encodeURIComponent(tr)}`;
  if (sd) url += `&startDate=${encodeURIComponent(sd)}`;
  if (ed) url += `&endDate=${encodeURIComponent(ed)}`;
  try {
    const res = await fetch(url);
...
async function refreshTopFavoriteChart(showAlert = true) {
  ...
  if (!validateFilterInputs(sd, ed)) return;
  let url = `${ocopApi}analytics-getTopProductsByFavorites?top=5&timeRange=${encodeURIComponent(tr)}`;
  if (sd) url += `&startDate=${encodeURIComponent(sd)}`;
  if (ed) url += `&endDate=${encodeURIComponent(ed)}`;
  try {
    const res = await fetch(url);
...
document.addEventListener("DOMContentLoaded", () => {  
  ...
  document.getElementById("compareProductsBtn")?.addEventListener("click", () => {
    ...
    const productIds = Array.from(select.selectedOptions).map(opt => opt.value);
  ...
  window.addEventListener('languageChanged', () => {  
    ...
    ids.length ? refreshCompareProductsChart(ids, false) : drawCompareProductsChart([]);
  });
...
(function () {
  ...
  conn.on("ChartAnalytics", () => {
    console.log("[SignalR] ChartAnalytics received.");  
