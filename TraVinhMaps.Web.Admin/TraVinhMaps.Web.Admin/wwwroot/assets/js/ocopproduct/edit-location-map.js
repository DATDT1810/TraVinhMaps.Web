...
function initializeEditMap(lat, lng) {
    ...
    addMapEventListeners(editMap, editMarker, behavior, 'latitude', 'longitude', 'locationAddress');
}
...
function initializeAddMap() {
    ...
    addMapEventListeners(addMap, addMarker, behavior, 'latitudeAdd', 'longitudeAdd', 'locationAddressAdd');
    updateFormFieldsFromMap(defaultCoords, 'latitudeAdd', 'longitudeAdd', 'locationAddressAdd');
}
        ...
        if (result.items.length > 0) {
            result.items.forEach(item => {
                ...
                suggestionItem.addEventListener('click', (e) => {
                    ...
                    suggestionsContainer.innerHTML = '';
                    suggestionsContainer.style.display = 'none';
                    if (addressId === 'locationAddress') {
                        updateMapFromAddress(editMap, editMarker, 'locationAddress', 'latitude', 'longitude');
                    } else {
                        updateMapFromAddress(addMap, addMarker, 'locationAddressAdd', 'latitudeAdd', 'longitudeAdd');
                    }
                });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchAddressBtn) {
        searchAddressBtn.addEventListener('click', () => updateMapFromAddress(editMap, editMarker, 'locationAddress', 'latitude', 'longitude'));
    }
    ...
    if (locationAddressInput) {
        locationAddressInput.addEventListener('input', () => {
            ...
            debounceTimer = setTimeout(() => {
                getAddressSuggestions(locationAddressInput.value, editMap, 'addressSuggestions', 'locationAddress');
            }, 300);
        });
    }
    ...
    if (searchAddressBtnAdd) {
        searchAddressBtnAdd.addEventListener('click', () => updateMapFromAddress(addMap, addMarker, 'locationAddressAdd', 'latitudeAdd', 'longitudeAdd'));
    }

    const locationAddressInputAdd = document.getElementById('locationAddressAdd');
    if (locationAddressInputAdd) {
        locationAddressInputAdd.addEventListener('input', () => {
            clearTimeout(debounceTimer);
            debounceTimer = setTimeout(() => {
                getAddressSuggestions(locationAddressInputAdd.value, addMap, 'addressSuggestionsAdd', 'locationAddressAdd');
            }, 300);
        });
    }
    ...
    document.addEventListener('click', function(event) {
        ...
        if (suggestionsContainer && !suggestionsContainer.contains(event.target) && event.target !== locationAddressInput) {
            suggestionsContainer.style.display = 'none';
        }
